...
: "${FAXCORE_DELEGATE_USERNAME:?Set FAXCORE_DELEGATE_USERNAME first.}"
: "${FAXCORE_UPLOAD_FILE:?Set FAXCORE_UPLOAD_FILE first.}"

if [ "${FAXCORE_CONFIRM_DESTRUCTIVE:-}" != "true" ]; then
  echo "Set FAXCORE_CONFIRM_DESTRUCTIVE=true to delegate a message." >&2
  exit 1
fi
...
UPLOADED_ID="$(printf '%s' "${UPLOAD_RESPONSE}" | sed -n 's/.*"id"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' | head -n 1)"
UPLOADED_FILE_NAME="$(printf '%s' "${UPLOAD_RESPONSE}" | sed -n 's/.*"fileName"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' | head -n 1)"
...
curl --fail-with-body --silent --show-error \
  ...
  --data '{
    "message": {
      "username": "'"${FAXCORE_DELEGATE_USERNAME}"'",
      "recipients": [
        {
      ...
      "subject": "'"${FAXCORE_SUBJECT:-FaxCore delegated sample fax}"'",
      "note": "'"${FAXCORE_NOTE:-Sent from the curl delegate sample.}"'",
