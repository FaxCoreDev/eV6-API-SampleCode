...
if [ "${FAXCORE_CONFIRM_DESTRUCTIVE:-}" != "true" ]; then
  echo "Set FAXCORE_CONFIRM_DESTRUCTIVE=true to update a contact." >&2
  exit 1
fi
...
curl --fail-with-body --silent --show-error \
  --request POST \
  --url "${FAXCORE_BASE_URL}/api/contact/update" \
  --header "Authorization: Bearer ${ACCESS_TOKEN}" \
  --header "Accept: application/json" \
  --header "Content-Type: application/json" \
  --data '{
    "contactID": '"${FAXCORE_CONTACT_ID}"',
    ...
    "description": "'"${FAXCORE_CONTACT_DESCRIPTION:-Updated from curl sample}"'",
    ...
    "addressList": [
      {
        ...
        "addressID": '"${FAXCORE_CONTACT_ADDRESS_ID:-0}"'
      }
    ]
  }'
