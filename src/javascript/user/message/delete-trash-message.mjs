...
if (process.env.FAXCORE_CONFIRM_DESTRUCTIVE !== 'true') {
  throw new Error('Set FAXCORE_CONFIRM_DESTRUCTIVE=true to permanently delete trash messages.');
}
...
const result = await client.delete('/api/message/delete/trash', {
  messageID: messageID.split(',').map((value) => value.trim()).filter(Boolean)
});

printResult('Delete trash message', result);
